...
export default function MapOverlay() {
  ...
  return (
    <div className="relative h-screen w-screen bg-gray-900 overflow-hidden text-white">
        ...
        <>
          <div className="absolute top-6 right-6 z-10">
            <button 
              ...
              className="bg-gray-900/90 backdrop-blur-xl border border-white/10 p-3 rounded-full shadow-2xl hover:bg-gray-800 transition-colors active:scale-95"
            >
          ...
          {status === 'countdown' && (
            <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
              <div className="flex flex-col items-center">
                <span className="text-2xl font-bold tracking-widest mb-4">PARTENZA TRA</span>
          ...
          <div className="absolute bottom-12 left-0 right-0 px-6 z-10 flex gap-4 justify-center">
            {status === 'idle' && (
              <button onClick={startCountdown} className="flex-1 max-w-[200px] py-5 rounded-3xl font-black text-lg text-white bg-blue-600 border-b-4 border-blue-800 hover:bg-blue-500 active:scale-95 shadow-2xl tracking-wide">
                START
              </button>
            )}
            {status === 'tracking' && (
              <>
                <button onClick={pauseTracking} className="w-16 h-16 rounded-full flex items-center justify-center font-black shadow-2xl bg-gray-800 border-b-4 border-gray-900">
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z"/></svg>
                </button>
                <button onClick={handleEndJourney} className="flex-1 max-w-[200px] py-5 rounded-3xl font-black text-lg bg-red-600 border-b-4 border-red-800 hover:bg-red-500 active:scale-95 shadow-2xl tracking-wide">STOP</button>
              </>
            )}
            {status === 'paused' && (
              <button onClick={resumeTracking} className="flex-1 max-w-[200px] py-5 rounded-3xl font-black text-lg text-black bg-yellow-500 border-b-4 border-yellow-700 hover:bg-yellow-400 active:scale-95 shadow-2xl tracking-wide">RESUME</button>
            )}
          </div>
        </>
      )}
      ...
      {showReport && tripStats && (
        <div className="absolute inset-0 z-50 bg-gray-900/95 backdrop-blur-md flex flex-col items-center justify-between p-6 animate-in fade-in">
          ...
          <div className="w-full max-w-md mb-8 flex flex-col gap-3">
            <button onClick={handleSaveToCloud} disabled={isSaving} className={`w-full py-5 rounded-3xl font-black text-lg transition-all shadow-xl flex justify-center items-center gap-2 ${isSaving ? 'bg-blue-800 text-blue-300' : 'bg-blue-600 hover:bg-blue-500 border-b-4 border-blue-800'}`}>
              {isSaving ? 'INVIO DATI...' : 'SALVA NEL CLOUD'}
            </button>
            <button onClick={() => setShowReport(false)} disabled={isSaving} className="w-full py-4 rounded-3xl font-bold text-gray-400 hover:text-white transition-colors">Scarta Dati</button>
          </div>
        </div>
      )}
      ...
      {/* --- MENU LATERALE --- */}
      {activeView === 'menu' && (
        <div className="absolute inset-0 z-50 bg-gray-900/98 flex flex-col p-6 animate-in fade-in zoom-in-95">
          <div className="flex justify-between items-center mt-8 mb-10">
            <h2 className="text-4xl font-black tracking-tighter text-blue-500">Road<span className="text-white">Record</span></h2>
            <button onClick={() => setActiveView('map')} className="p-2 bg-gray-800 rounded-full text-gray-400 hover:text-white">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>
          
          <div className="flex flex-col gap-4">
            <button onClick={() => setActiveView('history')} className="flex items-center gap-4 bg-gray-800 p-5 rounded-2xl border border-white/5 active:scale-95 transition-all text-left">
            ...
            <button onClick={() => setActiveView('settings')} className="flex items-center gap-4 bg-gray-800 p-5 rounded-2xl border border-white/5 active:scale-95 transition-all text-left">
      ...
      {/* --- 5.1.1 IMPOSTAZIONI --- */}
      {activeView === 'settings' && (
        <div className="absolute inset-0 z-50 bg-gray-900 flex flex-col p-6 animate-in slide-in-from-right">
          <div className="flex items-center gap-4 mt-8 mb-8">
            <button onClick={() => setActiveView('menu')} className="p-2 bg-gray-800 rounded-full text-gray-400">
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-gray-800 p-5 rounded-2xl flex justify-between items-center border border-white/5">
              <div>
                <p className="font-bold">Alta Precisione GPS</p>
                <p className="text-xs text-gray-400">Consuma più batteria</p>
              </div>
            ...
            <div className="bg-gray-800 p-5 rounded-2xl flex justify-between items-center border border-white/5">
              <div>
                ...
                <p className="text-xs text-gray-400">Per guidare di notte</p>
              </div>
              <input type="checkbox" defaultChecked className="w-6 h-6 accent-blue-500 rounded-md" />
            </div>
            <div className="bg-gray-800 p-5 rounded-2xl flex justify-between items-center border border-white/5">
              <div>
                <p className="font-bold">Unità di Misura</p>
              </div>
              <select className="bg-gray-900 border border-gray-700 rounded-lg p-2 text-sm font-bold focus:outline-none">
                <option>KM/H</option>
                <option>MPH</option>
              </select>
            </div>
          ...
          <div className="flex-1 overflow-y-auto pb-10 flex flex-col gap-4">
            {history.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-500 gap-2">
                ...
                <p className="text-sm">Inizia a guidare per salvare i percorsi.</p>
              </div>
            ) : (
              history.map((trip) => (
                <div 
                  key={trip.id} 
                  onClick={() => { setSelectedTrip(trip); setActiveView('tripDetail'); }}
                  className="bg-gray-800 rounded-2xl p-3 flex gap-4 border border-white/5 shadow-lg active:scale-[0.98] transition-all cursor-pointer hover:bg-gray-750"
                >
                  <div className="w-24 h-24 rounded-xl overflow-hidden bg-gray-900 flex-shrink-0 border border-white/10">
      ...
      {/* --- 5.1.3 DETTAGLIO STORICO (QUANDO CLICCHI UN VIAGGIO) --- */}
      {activeView === 'tripDetail' && selectedTrip && (
        <div className="absolute inset-0 z-50 bg-gray-900 flex flex-col animate-in zoom-in-95 duration-200">
          ...
          <div className="relative h-[45%] w-full bg-black">
            ...
            <div className="absolute top-0 left-0 right-0 p-6 pt-12 bg-gradient-to-b from-black/80 to-transparent flex justify-between">
              <button onClick={() => setActiveView('history')} className="p-3 bg-gray-900/80 backdrop-blur-md rounded-full text-white shadow-lg active:scale-95">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
              </button>
              <div className="bg-blue-600/90 backdrop-blur-md px-4 py-2 rounded-full font-bold shadow-lg">
                {selectedTrip.date}
              </div>
            </div>
          ...
          <div className="flex-1 p-6 -mt-8 relative z-10 flex flex-col gap-4">
            <div className="bg-gray-800 p-6 rounded-3xl border border-white/5 shadow-2xl flex flex-col gap-6">
              <div className="flex justify-between items-end border-b border-gray-700 pb-6">
                <div>
                  <p className="text-gray-400 text-sm font-bold uppercase mb-1">Distanza Percorsa</p>
                  <p className="text-5xl font-black text-white">{selectedTrip.distance.toFixed(2)} <span className="text-xl text-gray-500 font-bold">km</span></p>
                </div>
                <div className="text-right">
                  <p className="text-gray-400 text-sm font-bold uppercase mb-1">Tempo</p>
                  <p className="text-2xl font-bold text-blue-400">{formatTime(selectedTrip.time)}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                 ...
                 <div>
                   <p className="text-gray-500 text-xs font-bold uppercase mb-1">Velocità Massima</p>
                   <p className="text-2xl font-black text-red-400">{selectedTrip.maxSpeed.toFixed(1)} <span className="text-sm text-gray-500 font-bold">km/h</span></p>
                 </div>
              </div>
            </div>
          </div>
